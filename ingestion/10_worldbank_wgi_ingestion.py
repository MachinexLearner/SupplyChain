...
# MAGIC %md
# MAGIC # World Bank WGI Ingestion
# MAGIC
# MAGIC **Executive summary:** Ingests Worldwide Governance Indicators (political stability, government effectiveness, rule of law, etc.) from the World Bank API into raw and bronze. Used as structured governance/political risk indicators.
# MAGIC
# MAGIC **Data Source**: https://api.worldbank.org/v2 — World Bank Indicators API. WGI series use same endpoint: `country/all/indicator/{CODE}?format=json&date=...&per_page=10000`.
# MAGIC
# MAGIC **Target Tables** (Unity Catalog):
# MAGIC - `supply_chain.raw.worldbank_wgi` - Raw WGI observations
# MAGIC - `supply_chain.bronze.worldbank_wgi` - Cleaned WGI with typed columns
# MAGIC
...
# MAGIC %md
# MAGIC Tables: `supply_chain.raw.worldbank_wgi`, `supply_chain.bronze.worldbank_wgi`. Idempotent merge on (source, indicator_code, country_code, as_of_date).
